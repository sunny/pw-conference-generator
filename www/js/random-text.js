...
export default class RandomText {
  ...
  #fixTypos(text) {
    return ` ${text} `
      ...
      .replace(/ double (le |la |ma |l’)/g, " double ")
      .replace(" je dois se ", " je dois me ")
      .replace(" du HTML5 ", " d’HTML5")
      .replace(" le Firefox ", " Firefox ")
      .replace(" le Google ", " Google ")
      .replace(/ le HTTP([S]) /g, " HTTP$1 ")
      .replace(" mon Apple ", " Apple ")
      ...
      .replace(/ le ([aAeEiIoOuUué][\w])/g, " l’$1")
      .replace(/ de ([aAeEiIoOuUué][\w])/g, " d’$1")
      .replace(/ le l’/g, " l’")
      .replace(/ (: .*): /g, " $1, ")
